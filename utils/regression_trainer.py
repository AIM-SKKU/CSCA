...
class RegTrainer(Trainer):
    def setup(self):
        ...
        if args.dataset == 'ShanghaiTechRGBD':
            ...
            args.data_dir = os.path.join(args.data_dir, 'ShanghaiTechRGBD')
            self.datasets = {x: Crowd(os.path.join(args.data_dir, '%s_data' % x),
                                      args.crop_size,
                                      args.downsample_ratio,
                                      x) for x in ['train', 'val', 'test']}  # crows output: input[rgb,T], target, name
            ...
            Crowd = Crowd_RGBTCC
            args.data_dir = os.path.join(args.data_dir, 'bayes-RGBT-CC')
            self.datasets = {x: Crowd(os.path.join(args.data_dir, x),
                                          args.crop_size,
                                          args.downsample_ratio,
                                          x) for x in
                                 ['train', 'val', 'test']}  # crows output: input[rgb,T], target, name
            self.dataloaders = {x: DataLoader(self.datasets[x],
                                                  collate_fn=(train_collate
                                                              if x == 'train' else default_collate),
                                                  batch_size=(args.batch_size
